...
var coordsArr = [];
var flagNavigate = 0;
var sourceInputElement;
var destinationInputElement;
...
function initMap() {
   ...
   map.addListener('zoom_changed', function() {
        // alert(prevZoom);
         ...
         var currZoom = Number(map.getZoom());
         console.log(currZoom);
          if(currZoom < 14 && currZoom >=9)
          {
            
            //alert(currZoom);
           // alert((Number(prevZoom) - Number(currZoom)));
            scalesize = scalesize + ((prevZoom - currZoom)/15);
           // alert(scalesize);
            // console.log(map.getZoom(),"  ",scalesize);
            prevZoom = currZoom;
          }
        });
  ...
  autocompleteend = new google.maps.places.Autocomplete(
      /* @type {!HTMLInputElement}*/ (destinationInputElement),
      {types: ['geocode']});
  autocompletestart = new google.maps.places.Autocomplete(
       /*@type {!HTMLInputElement} */(sourceInputElement),
      {types: ['geocode']});
...
function getSourceInputElement(elem)
{
  sourceInputElement =elem;
 
}

function getDestinationInputElement(elem)
{
  destinationInputElement = elem;
  //geolocate("end");
}
...
function showCurrentPosition(position) {
 var userLatLng = new google.maps.LatLng(position.coords.latitude, position.coords.longitude);
       var iconCircle = google.maps.SymbolPath.CIRCLE;
...
function writeAddressName(latLng) {
        ...
        function(results, status) {
         ...
         // sourceInputElement.value =String(addressInText); 
          }
          else
          {
...
//generates path from source to destination via current location
 var calculateAndDisplayRouteFromCurrentLoc =  function(directionsService, directionsDisplay) {
  ...
  directionsService.route({
    origin: sourceInputElement.value,
    //waypoints: waypts,
    destination: destinationInputElement.value,
    travelMode: 'DRIVING'
  ...
  console.log("cnt- ",x); 
    ...
    else
    {
      ...
      var curlng = Number(coords[1]); 
       curLatLngPosition = {lat: curlat, lng: curlng};
...
," M 75.86 33.56 C 76.70 33.13 77.52 32.68 78.35 32.22 C 79.27 33.03 80.17 33.86 81.00 34.76 L 81.00 37.05 C 79.23 37.25 77.46 37.45 75.69 37.64 C 75.64 43.18 76.33 48.74 75.40 54.24 C 73.86 54.32 72.33 54.40 70.79 54.48 C 70.81 59.98 70.12 65.49 70.68 70.97 C 70.42 70.68 69.88 70.09 69.61 69.79 C 69.41 64.65 69.82 59.51 69.74 54.37 C 71.32 54.32 72.90 54.27 74.48 54.23 C 75.00 48.46 75.96 42.63 74.91 36.85 C 76.67 36.63 78.43 36.40 80.18 36.06 C 78.95 35.03 77.90 32.22 75.86 33.56 Z"];
       imgColors = ["#ffffff","#fd9b9b","#990000","#ff0000","#080808","#ff0000","#080808","#ff0000","#990000","#fd9b9b","#fd9b9b","#fd9b9b","#fd9b9b"];
      var startIconMarker; 
      ...
      if(objArr.length > 0)
      {
          ...
          {
           objArr[j].setMap(null);
          }
      }
      
      //  objArr = [];
      // var newScaleSze = scalesize;
      ...
      //     icon:{
      //       path:imgString[i],
      //      // path: 'assets/icon/towtruck.png',
      //       fillOpacity: 1,
      //       rotation:rotate,
      //        fillColor: imgColors[i],
      ...
      //Inserting waypoints
      var wayptsArr = [];
      if(coordsIndex+1 < coordsArr.length)
      {
        wayptsArr.push({location:coordsArr[coordsIndex+1],stopover:false});
      }

      var waypt = [{location:"17.487260000000003,78.38561",stopover:false}];
     ...
     // alert(rotate);
      ...
      //Setting automatic bounds
      var current_bounds = map.getBounds();
      var marker_pos;
      if(coordsIndex+2 < coordsArr.length)
      {
        // marker_pos = "("+coordsArr[coordsIndex+2]+")";
        var coordinates = coordsArr[coordsIndex+2].split(",");
        var curlatt = Number(coordinates[0]);
        var curlngg = Number(coordinates[1]); 
        marker_pos = {lat: curlatt, lng: curlngg};
     
      }
      else  
      {
        // marker_pos = startIconMarker.getPosition();
        var coordinates = coordsArr[coordsIndex].split(",");
        var curlatt = Number(coordinates[0]);
        var curlngg = Number(coordinates[1]); 
        marker_pos = {lat: curlatt, lng: curlngg};
      }
        var coordinatesForIconPos = coordsArr[coordsIndex].split(",");
        var curlattForIconPos = Number(coordinatesForIconPos[0]);
        var curlnggForIconPos = Number(coordinatesForIconPos[1]); 
        var IconMarkerPos = {lat: curlattForIconPos, lng: curlnggForIconPos};
       // var IconMarkerPos = iconMarkerPos.getPosition();
     // var  IconMarkerPos = startIconMarker.getPosition();
      ...
      else if(!current_bounds.contains( IconMarkerPos))
      {
        
        var new_bounds = current_bounds.extend( IconMarkerPos );
          map.fitBounds( new_bounds ); 
      }
      ...
      {
        demoFlagNavigate=1;
        demoInterval = setInterval(function(){ demoNavigation(); }, 2000);
